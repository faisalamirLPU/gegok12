        ...
        <form method="POST" action="{{ route('finance.payments.store', $fee->id) }}"
            ...
            <div class="border-b p-5 bg-gray-50/50">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {{-- Invoice Number --}}
                    <div>
                        <label class="block text-xs font-medium text-gray-500 uppercase tracking-wide mb-1">Invoice
                            Number</label>
                    ...
                    {{-- Balance --}}
                    <div>
                        <label class="block text-xs font-medium text-gray-500 uppercase tracking-wide mb-1">Outstanding
                            Balance</label>
                        <div
                            class="w-full border border-red-100 rounded px-4 py-2 bg-red-50 text-red-700 text-sm font-bold">
            ...
            <div class="p-5">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {{-- Amount --}}
                    <div>
                        <label class="block text-xs font-medium text-gray-500 uppercase tracking-wide mb-1">Collection
                            Amount <span class="text-red-500">*</span></label>
                        <input type="number" step="0.01" name="amount" required placeholder="Enter amount to collect"
                            class="w-full border border-gray-300 rounded px-4 py-2 text-sm focus:border-blue-500 focus:ring-0">
                    </div>

                    {{-- Payment Method --}}
                    <div>
                        <label class="block text-xs font-medium text-gray-500 uppercase tracking-wide mb-1">Payment Method
                    ...
                    <div class="md:col-span-2">
                        <label class="block text-xs font-medium text-gray-500 uppercase tracking-wide mb-1">Transaction
                            Reference</label>
                        <input type="text" name="transaction_id" placeholder="Enter transaction ID or cheque number"
                            class="w-full border border-gray-300 rounded px-4 py-2 text-sm focus:border-blue-500 focus:ring-0">
                    </div>

                    {{-- Remarks --}}
                    <div class="md:col-span-2">
                        <label class="block text-xs font-medium text-gray-500 uppercase tracking-wide mb-1">Administrative
                            Remarks</label>
