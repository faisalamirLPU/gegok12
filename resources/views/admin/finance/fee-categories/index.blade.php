                    ...
                    @forelse($feeCategories as $category)
                        <tr class="hover:bg-gray-50 transition fee-row">
                            ...
                            <td class="px-5 py-3 text-center">
                                @if($category->trashed())
                                    <span class="inline-flex items-center px-2 py-0.5 rounded-none text-[9px] font-bold bg-gray-100 text-gray-500 uppercase tracking-wider border border-gray-200">Archived</span>
                            ...
                            <td class="px-5 py-3 text-center">
                                <div class="flex items-center justify-center gap-2">
                                    @if($category->trashed())
                                        <form action="{{ route('finance.fee-categories.restore', $category->id) }}" method="POST" class="inline">
                                            @csrf
                                            <button type="submit" class="p-1.5 text-blue-600 hover:bg-blue-50 transition border border-transparent hover:border-blue-100 rounded" title="Restore Category" onclick="return confirm('Restore this archived fee category?')">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 12l6 6 10-10" />
                                                </svg>
                                            </button>
                                        </form>
                                    @else
                                        ...
                                        <form action="{{ route('finance.fee-categories.destroy', $category->id) }}" method="POST" class="inline">
                                            @csrf @method('DELETE')
                                            <button type="submit" class="p-1.5 text-red-500 hover:bg-red-50 transition border border-transparent hover:border-red-100 rounded"
                                                    title="Archive Category" onclick="return confirm('This category has historical invoice references. Deleting it may affect future billing structures. Historical invoices will remain preserved. Do you want to archive this category instead?')">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
